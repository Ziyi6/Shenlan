...
void dios_ssp_aec_firfilter_detect(objFirFilter *srv)
{
	...
	for (ch = AEC_LOW_CHAN; ch < AEC_HIGH_CHAN; ch++)
	{
		...
		else if ((srv->mse_mic_in[ch] > srv->mse_adpt[ch] * MSE_RATIO_OUT_IN) && (srv->mse_adpt[ch] < FILTER_COPY_FAC * srv->mse_main[ch]))
		{
			for (i_ref = 0; i_ref < srv->ref_num; i_ref++)
			{
				for (i = 0; i < srv->num_main_subband_adf[ch]; i++)
		...
		else if ((srv->mse_mic_in[ch] > srv->mse_main[ch] * MSE_RATIO_OUT_IN) && (srv->mse_main[ch] < FILTER_COPY_FAC * srv->mse_adpt[ch]))
		{
			for (i_ref = 0; i_ref < srv->ref_num; i_ref++)
			{
				for (i = 0; i < srv->num_main_subband_adf[ch]; i++)
 ...
 * Reference signal is stored in srv->stack_sigIn_adf[][][].  x(n)
 * Error signal is stored in srv->err_adf[].  e(n)
...
void nlms_complex(int ch, objFirFilter *srv, int i_ref)
{
	...
	float mu = srv->weight[ch]; // stepsize
	int filter_order = srv->num_main_subband_adf[ch];
	xcomplex error = srv->err_adf[ch];
	xcomplex error_conj = complex_conjg(error);
	float energy = 0.0;
	for (int i = 0; i < filter_order; i++)
	{
		xcomplex ref = srv->stack_sigIn_adf[i_ref][ch][i];
		energy = energy + complex_abs2(ref);
	}
	for (int i = 0; i < filter_order; i++)
	{
		xcomplex ref = srv->stack_sigIn_adf[i_ref][ch][i];
		xcomplex coeff = complex_gen((float)2 * mu / energy, (float)0);
		xcomplex sum2 = complex_mul(srv->stack_sigIn_adf[i_ref][ch][i], error_conj);
		sum2 = complex_mul(sum2, coeff);
		srv->adf_coef[i_ref][ch][i] = complex_add(srv->adf_coef[i_ref][ch][i], sum2);
	}
	// end NLMS
}
